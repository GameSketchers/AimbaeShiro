...
window[cheatInstanceId] = function() {
    ...
    class AimbaeShiro {
        ...
        onProcessInputs(inputPacket, player) {
            const gameInputIndices = { frame: 0, shoot: 1, scope: 2, xdir: 3, ydir: 4, moveDir: 5, reload: 6 };
            ...
            if (this.settings.autoReload && this.me.ammos[this.me.weapon.ammo] === 0 && !this.me.reloadTimer) {
            ...
            if (this.settings.aimbotEnabled && (!this.settings.aimbotOnRightMouse || this.rightMouseDown)) {
                ...
                .filter(p => p.health > 0 && p.inView && this.isDefined(p) && !p.isYou && p.active && p.health > 0 && (!this.settings.aimbotTeamCheck || !this.isTeam(p)))
                .sort((a, b) => this.getDistance(this.me, a) - this.getDistance(this.me, b));

                this.controls.target = null;
                /*let potentialTargets = this.game.players.list.filter((p) => !p.isYTMP && p.hasOwnProperty('npos') && (!this.settings.frustumCheck || this.isPointInFrustum(p.npos)) && (this.me.team === null || p.team !== this.me.team) && p.health > 0 && p.inView).sort((a, b) => this.getDistance2D(this.me.x, this.me.z, a.npos.x, a.npos.z) - this.getDistance2D(this.me.x, this.me.z, b.npos.x, b.npos.z));
                let target = potentialTargets[0];*/

                for (const p of potentialTargets) {
                    if (!this.settings.aimbotWallCheck || p.inView) {
                        target = p;
                        break;
                    }
                }
            ...
            if (target) {
                ...
                const ydir = (this.getDirection(this.me.z, this.me.x, targetPos.z, targetPos.x) || 0) * 1000;
                const xdir = ((this.getXDirection(this.me.x, this.me.y, this.me.z, targetPos.x, targetPos.y - targetPos.crouchVal * 3 + this.me.crouchVal * 3 + this.settings.aimOffset, targetPos.z) || 0) - 0.3 * this.me.recoilAnimY) * 1000;
                ...
                // süper silent için
                /*inputPacket[gameInputIndices.ydir] = ydir * 1000;
                inputPacket[gameInputIndices.xdir] = (xdir - 0.3 * this.me.recoilAnimY) * 1000;*/

                if (this.settings.autoFireEnabled && this.me.reloadTimer === 0 && this.me.didShoot && this.settings.aimbot === 'quickscope' && this.me.aimVal > 0) {
        ...
        getTargetPosition(t){const e=this.PLAYER_HEIGHT/2.5;return{x:t.x,y:t.y-t.crouchVal*this.CROUCH_FACTOR+e,z:t.z}}
